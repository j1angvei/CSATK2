...
//        commands.add(OsCmd.addPythonPath(CONF.getSwDestFolder(Software.DEEPTOOLS)));
        ...
        //usage:  computeMatrix scale-regions -S <bigwig file> -R <bed file> -b 1000
        //using bigwig generated by deeptools
        String inTss = ConfigInitializer.getPath(Sub.GENOME) + CONF.getGenome(experiment.getGenomeCode()).getAnnotation() + Constant.SFX_UCSC_GENE_BED;
        String inBw = ConfigInitializer.getPath(Out.BIGWIG_DT) + experiment.getCode() + Constant.SFX_DT_BIG_WIG;
...
//        String exeParseGtf = CONF.getSwExecutable(Software.HOMER) + Constant.EXE_HOMER_PARSE_GTF;
        ...
        String midGenePred = annoFile + Constant.SFX_UCSC_GENEPRED;
        String outTss = annoFile + Constant.SFX_UCSC_GENE_BED;

        //exe ucsc
        String exeUcsc = CONF.getSwExecutable(Software.UCSC);
        String exeGenePredToBed = exeUcsc + Constant.EXE_UCSC_GENEPRED_TO_BED;
        String exeGtfToGenePred = exeUcsc + Constant.EXE_UCSC_GTF_TO_GENEPRED;// gtfToGenePred gtf genePred
        String exeGff3ToGenePred = exeUcsc + Constant.EXE_UCSC_GFF3_TO_GENEPRED;//gff3ToGenePred inGff3 outGp

        //convert annotation file GFF3 or GTF to genePred file
        if (annoFormat.equals("gff3")) {
            commands.add(String.format("%s %s %s", exeGff3ToGenePred, annoFile, midGenePred));
        } else if (annoFormat.equals("gtf")) {
            commands.add(String.format("%s %s %s", exeGtfToGenePred, annoFile, midGenePred));
        }

        //convert genePred file to bed format file
        //usage:  genePredToBed in.genePred out.bed
        commands.add(String.format("%s %s %s", exeGenePredToBed, midGenePred, outTss));
