...
public class SwCmd {
    ...
    public static String[] getExperimentCommands(Function function, Experiment exp) {
        switch (function) {
            ...
            case MATRIX:
                return SwCmd.computeMatrix(exp);
            default:
...
//
//        String inPrefix = ConfigInitializer.getPath(Out.PEAK_CALLING) + experiment.getCode();
//        String inPeak = inPrefix + (experiment.isBroadPeak() ? Constant.SFX_BROAD_PEAKS : Constant.SFX_NARROW_PEAKS);
//
//        String matrix = ConfigInitializer.getPath(Out.MATRIX) + experiment.getCode() + Constant.SFX_DT_TSS_MATRIX;
//        String outPrefix = ConfigInitializer.getPath(Out.PEAK_HEATMAP) + experiment.getCode();
//        String outHeatmap = outPrefix + Constant.PNG_DT_HEATMAP;
//
//        commands.add(String.format("%s -m %s -out %s ",
//                exePlotHeatmap, matrix, outPngHeatmap));
//    }
