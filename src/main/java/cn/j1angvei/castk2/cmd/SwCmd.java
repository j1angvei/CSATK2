...
public class SwCmd {
    ...
    public static String genomeIndex(Genome genome) {
        return String.format("%s index -p %s %s",
                ...
                CONF.getOutDirectory(OutType.IDX_GENOME) + genome.getCode(),
    ...
    public static String[] qcRawReads(Experiment experiment) {
        ...
        cmd.add(String.format("%s -o %s -t %d %s",
                ...
                CONF.getOutDirectory(OutType.QC_RAW),
                SwUtil.THREAD_NUMBER,
                CONF.getSubDirectory(SubType.INPUT) + experiment.getFastq1())
        );
    ...
    public static String trimReads(Experiment experiment) {
        ...
        String fastq1 = CONF.getSubDirectory(SubType.INPUT) + experiment.getFastq1();
        String inputPrefix = CONF.getOutDirectory(OutType.PARSE_ZIP) + experiment.getCode();
        String outputPrefix = CONF.getOutDirectory(OutType.TRIM) + experiment.getCode();
        ...
        if (experiment.getFastq2() == null) {
            ...
            return String.format("%s -Xmx256m -jar %s SE -threads %d %s %s %s " + PARAM,
                    ...
                    SwUtil.THREAD_NUMBER,
                    phred,
                    CONF.getSubDirectory(SubType.INPUT) + fastq1,
                    outputPrefix + ".fastq",
                    faFile,
                    minLen);
        } else {
            //pair end
            String fastq2 = CONF.getSubDirectory(SubType.INPUT) + experiment.getFastq2();
            return String.format("%s -Xmx256m -jar %s PE -threads %d %s %s %s %s %s %s %s " + PARAM,
                    ...
                    phred,
                    CONF.getSubDirectory(SubType.INPUT) + fastq1,
                    CONF.getSubDirectory(SubType.INPUT) + fastq2,
    ...
    public static String[] qcCleanReads(Experiment experiment) {
        ...
        cmd.add(String.format("%s -o %s -t %d %s",
                CONF.getSoftwareExecutable(SwType.FASTQC),
                CONF.getOutDirectory(OutType.TRIM),
                SwUtil.THREAD_NUMBER,
                CONF.getOutDirectory(OutType.QC_CLEAN) + experiment.getFastq1())
        );
        if (experiment.getFastq2() != null) {
            cmd.add(String.format("%s -o %s -t %d %s",
                    CONF.getSoftwareExecutable(SwType.FASTQC),
    ...
    public static String alignment(Experiment experiment) {
        return experiment.getFastq2() == null ?
                String.format("%s mem %s -t %d %s > %s",
                        CONF.getSoftwareExecutable(SwType.BWA),
                        CONF.getOutDirectory(OutType.IDX_GENOME) + experiment.getGenomeCode(),
                        SwUtil.THREAD_NUMBER,
                        CONF.getOutDirectory(OutType.TRIM) + experiment.getCode() + ".fastq",
                        CONF.getOutDirectory(OutType.ALIGNMENT) + experiment.getCode() + ".sam") :
                String.format("%s mem %s -t %d %s %s > %s",
                        CONF.getSoftwareExecutable(SwType.BWA),
                        CONF.getOutDirectory(OutType.IDX_GENOME) + experiment.getGenomeCode(),
                        SwUtil.THREAD_NUMBER,
                        CONF.getOutDirectory(OutType.TRIM) + experiment.getCode() + "_1.fastq",
                        CONF.getOutDirectory(OutType.TRIM) + experiment.getCode() + "_2.fastq",
                        CONF.getOutDirectory(OutType.ALIGNMENT) + experiment.getCode() + ".sam");
    }
    ...
    public static String convertSamToBam(Experiment experiment) {
        return String.format("%s view --threads %d -bS %s -o %s",
                ...
                SwUtil.THREAD_NUMBER,
                CONF.getOutDirectory(OutType.ALIGNMENT) + experiment.getCode() + ".sam",
                CONF.getOutDirectory(OutType.BAM_CONVERTED) + experiment.getCode() + ".bam");
    }
    ...
    public static String sortBam(Experiment experiment) {
        return String.format("%s sort %s --threads %d -o %s",
                ...
                CONF.getOutDirectory(OutType.BAM_CONVERTED) + experiment.getCode() + ".bam",
                SwUtil.THREAD_NUMBER,
                CONF.getOutDirectory(OutType.BAM_SORTED) + experiment.getCode() + ".bam");
    }
    ...
    public static String[] callPeaks(Experiment experiment) {
        ...
        if (gSize == 0) {
            gSize = FileUtil.countFileSize(CONF.getSubDirectory(SubType.INPUT) + genome.getFasta());
        ...
        if (experiment.getFastq2() == null) {
            ...
            cmd.add(String.format("%s callpeak -t %s -c %s -f BAM -g %d -n %s -B",
                    CONF.getSoftwareExecutable(SwType.MACS2),
                    CONF.getOutDirectory(OutType.BAM_SORTED) + experiment.getCode() + ".bam",
                    CONF.getOutDirectory(OutType.BAM_SORTED) + experiment.getControl() + ".bam",
                    gSize,
                    CONF.getOutDirectory(OutType.PEAK_CALLING) + experiment.getCode())
            );
        }
    ...
    public static String[] annotatePeaks(Experiment experiment) {
        ...
        cmd.add(String.format("%s %s %s %s %s",
                CONF.getSoftwareExecutable(SwType.HOMER),
                CONF.getOutDirectory(OutType.PEAK_CALLING) + experiment.getCode() + "_peaks.bed",
                CONF.getSubDirectory(SubType.INPUT) + genome.getFasta(),
                annoFormat,
